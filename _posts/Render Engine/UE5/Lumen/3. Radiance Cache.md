...
- 每帧trace更新probe的开销是有限的，因此需要评估probe的更新优先级以及trace开销
  - 更新优先级，基于 LastTracedFrameIndex, LastUsedFrameIndex, ClipmapIn=ex 来划分PriorityBucketIndex，bucket越靠前优先级越高。`GetPriorityBucketIndex`
...
- TraceResolution < RadianceCache.ProbeResolution：执行 upsample 逻辑
- 否则执行 downsample 逻辑

这里要resolve的数据包括，Radiance、HitDistance：

- upsample：一条trace光线对应一个cache区域 (UpsampleFactor x UpsampleFactor)
  - 在该区域所有texel都填充上trace光线的 Radiance、HitDistance。
- downsample：一个cache texel对应的一块 trace 光线区域 (DownsampleFactor x DownsampleFactor)
  - Radiance：trace区域取平均
  - HitDistance：trace区域取最小。保守的遮挡判断，可避免漏光。

### 3.4.3 Spatial Filter

`FilterProbeRadianceWithGatherCS` 一个pass执行 3x3x3 区域的filter，每个probe算上自身共需访问 6+1 = 7 个probe。如果每个轴向一个pass，即共3个pass，那么每个probe共需要访问9个probe。每个 work group 负责一个 trace probe。

filter 权重设计：

- OcclusionWeight 默认为 1.0：需要测试probe能否看到neighbor，如果被遮挡则丢弃neighbor样本，以减少漏光。主要策略是 neighbor <->probe 作为光线方向，查询该方向上的 neighbor visibility 信息，如果 visibility(光线hit distance) 小于 neighbor 与 probe 之间的距离，表示二者之间有遮挡。

  - probe -> neighbor 遮挡测试

    - 光线是从 probeTMin 后开始追踪，probeTMin 内没有visibility信息，因此需要对neighbor测试点做偏移
    ...
    - 测试方向：`float3 ToNeighborOcclusionPosition = NeighborOcclusionTestPosition - ProbeWorldCenter;`
      查询该方向上的 neighbor visibility `ProbeDepthForNeighborOcclusionPosition`
    - 如果 `ProbeDepthForNeighborOcclusionPosition < length(ToNeighborOcclusionPosition)`，表示中间有遮挡，OcclusionWeight = 0.0

  - neighbor -> probe 遮挡测试

    - 过程类似，这里偏移 probe 位置以及查询probe visibility。

- AngleWeight：基于 probe 到 neighbor hit 的方向与当前texel方向的夹角设计的权重，夹角越大权重越低，SpatialFilterMaxRadianceHitAngle 默认为 10
  ```c++
  float AngleWeight = 1.0f - saturate(NeighborAngle / SpatialFilterMaxRadianceHitAngle);
  ```

- 最终权重设计为 weight = OcclusionWeight * AngleWeight;

在过程中，还会将 hitDistance 与 neighbor 之间取最小，为了避免漏光。
...
这里的 irradiance 就是积分probe球面radiance。irradiance分辨率可能低于 radiance 分辨率，这里还涉及到 irradiance 分辨率与 radiance 分辨率之间的对齐。

遍历球面，使用 irradiance direction 与 radiance direction 之间 cos，执行积分，就是 diffuse 渲染方程的积分。irradiance direction 对应的是查询时的着色点法线方向。

### 3.4.5 Visibility

`PrepareProbeOcclusionCS`

准备 visibility 的一阶矩、二阶矩，thread group 大小为 `OcclusionProbeResolution`，默认为 `GLumenIrradianceFieldProbeOcclusionResolution (16)`

## 3.5 采样

world probe 提供两种采样，一种是采样 radiance，用于 ScreenProbe/Reflection/Volume，这是默认的Lumen流程；一种是采样 irradiance，用于 IrradianceFieldGather。
